...
const imageUrlToBase64 = async (url) => {
    ...
    try {
        ...
        // P5 FIX: detecta formato pelo MIME type real, não pela URL
        const mimeType = blob.type; // "image/png", "image/jpeg", "image/webp"
        return new Promise((resolve, reject) => {
            const reader = new FileReader();
            reader.onloadend = () => resolve({ base64: reader.result, mimeType });
...
export async function generateBudgetPDF({
    ...
    try {
        ...
        // C2 FIX: Sem fallback para localStorage legado — dados vêm sempre do Supabase
        const nomeEmpresa = company.company_name ?? "Sua Empresa";
        const cnpj = company.cnpj ?? "";
        const telefone = company.phone ?? "";
        const enderecoEmpresa = company.address ?? "";
        let logoSrc = company.logo_url ?? null;
        let logoImgFormat = "JPEG";

        if (logoSrc && logoSrc.startsWith("http")) {
            const result = await imageUrlToBase64(logoSrc);
            if (result) {
                logoSrc = result.base64;
                // P5 FIX: usa MIME type real para determinar o formato
                if (result.mimeType.includes("png")) logoImgFormat = "PNG";
                else if (result.mimeType.includes("webp")) logoImgFormat = "WEBP";
                else logoImgFormat = "JPEG";
            } else {
                logoSrc = null;
            }
        }
        ...
        if (layout === "modern") {
            ...
            if (logoSrc) {
                try {
                    doc.addImage(logoSrc, logoImgFormat, marginX, y, 30, 30);
                } catch (e) { /* silencioso */ }
            }
        ...
        // ============================================================
            ...
            if (logoSrc) {
                try {
                    doc.addImage(logoSrc, logoImgFormat, marginX, y, 25, 25);
                } catch (e) { /* silencioso */ }
            }
        ...
        // ============================================================
            ...
            if (logoSrc) {
                try {
                    doc.addImage(logoSrc, logoImgFormat, pageWidth / 2 - 15, y, 30, 30);
                } catch (e) { /* silencioso */ }
                y += 35;
            } else {
                y += 10;
        ...
        // ============================================================
            ...
            if (logoSrc) {
                try {
                    doc.addImage(logoSrc, logoImgFormat, marginX + 5, y - 5, 20, 20);
                } catch (e) { /* silencioso */ }
            }
